...
const Routes = () => {
  ...
  const getDeepLinkInfo = async () => {
    ...
    if (
      route === 'main' ||
      !url ||
      !Config?.IOS_DEEP_LINK ||
      !Config?.ANDROID_DEEP_LINK
    )
      return;
    let split =
      Platform.OS === 'ios'
        ? url.split(Config?.IOS_DEEP_LINK)
        : url.split(Config?.ANDROID_DEEP_LINK);
    let load = {
      ...auth,
      onboard: true,
  ...
  useEffect(() => {
    const unsubscribe = messaging().onMessage(
      async (message: PushNotificationData) => {
    ...
    try {
      ...
      const updateOptions: StartUpdateOptions | undefined = Platform.select({
        ios: {
          ...
          message:
            'There is a new version of MyEdge available on the App Store, do you want to update it?',
