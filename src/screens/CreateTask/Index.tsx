import { View,TouchableOpacity, Text, LayoutAnimation, LayoutChangeEvent, Keyboard } from 'react-native'
import moment from 'moment';
import React, { useState, useEffect, } from 'react'
import styles from './style'
import { P, KeyboardAwareWrapper, H1, PageLoader, Container } from '../../utills/components'
import AppColors from '../../utills/AppColors';
import CustomInput from '../../components/CustomInput/index';
import { Capitalize,ToastError, ToastSuccess, useAppDispatch } from '../../utills/Methods';
...
import { GET_TASKS, GET_TASK_BY_PK, GET_TASK_STATISTICS, TaskLoad } from '../../utills/payload';
...
const CreateTask = ({ route } : RootMenuScreenProps) => {
    ...
    } = useFetchTaskByPK(typeof task_id === "number" ? task_id : undefined) as useFetchTaskByPKProps
    ...
    const submitHandler = async () => {
        try {
            ...
            if(
               typeof about?.id !== "number"
            ) return

            let fd : TaskLoad = {
                title : data?.title,
                due_date: data?.due_date === 'Today' ? moment().toISOString(true) : data?.due_date ? moment(data?.due_date).format('YYYY-MM-DD[T]HH:mm:ss.SSS') : undefined,
                created_by: task?.created_by?.id || about?.id,
                assigned_to: data?.type === "Employee" && data?.assigned_to_id && typeof data?.assigned_to_id === "number" ? data?.assigned_to_id : data?.type === "Employee"  ? about?.id : null,
                department: data?.type === "Departments" && typeof data?.assigned_to_id === "number" ? data?.assigned_to_id : null,
                status: "To-do",
    ...
    useEffect(() => {
        ...
        setData({
            ...
            name: task?.department?.id && task?.department?.name ? Capitalize(task?.department?.name) : 
            task?.assigned_to?.id && task?.assigned_to?.first_name ? Capitalize(task?.assigned_to?.first_name) : "",
    ...
    return (
        <ScreenWrapper>
            ...
            />
           {
               loadingTask ? <PageLoader /> :  <View style={styles.mainViewContainer}>
                    ...
                    <KeyboardAwareWrapper scrollable={true}>
                        ...
                        >
                            <View style={styles.assign}>
                                ...
                                >
                                    {
                                        !data?.assigned_to_id ? <Ionicons name='person-add' size={15} color={AppColors.black3} /> : null
                                    }
                                    <P style={styles.btnIcon} numberOfLines={1}>
                                        {
                                            !data?.assigned_to_id ? "You" : data?.name ? Capitalize(data?.name) : ""
                                        }
                                    </P>
                                </TouchableOpacity>
                            </View>
                            <View style={styles.dueDate}>
                                ...
                                <Container width={50} direction="row" verticalAlignment='center'>
                                    <TouchableOpacity
                        ...
                        {
                            sub_tasks && Array.isArray(sub_tasks) ? sub_tasks.map((sub_task,i)=><CustomInput 
                                multiline={true}
                                mode={'flat'}
                                minHeight={5}
                                autoFocus={true}
                        ...
                        >
                            ...
                            {
                                isLoading || isLoadingEdit ? <ActivityIndicator size={width(5)} color={AppColors.green} /> : <TouchableOpacity
                                    onPress={submitHandler} >
                                    <H1 color={AppColors.green}>{task?.id ? "Save" : "Create"}</H1>
                                </TouchableOpacity>
                            }
                        </View>
                    ...
                    {
                        show ? <CustomCalendarModal 
                            show={show}
                            onDayPress={onDayPress}
                            date={
                                data?.due_date === "Today" ? moment().format("YYYY-MM-DD") : 
