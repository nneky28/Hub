...
export default function EditPhoto() {
  ...
  const {mutateAsync, isLoading: isDeleting} = useMutation(
    APIFunction.remove_photo,
  );

  const {mutateAsync: completeOnboarding, isLoading: onboarding} = useMutation(
    APIFunction.complete_user_onboarding,
  );
  ...
  const SvgCircle = () => (
    <Svg height={height(25)} width={height(25)}>
      <Rect
        height="100%"
        width="100%"
        ...
        <View style={styles.mainViewContainer}>
          <View>
            ...
            {about && !about.photo && !profilePicture ? (
              <ImgPlaceholder
                text={`${
                  about?.first_name ? Capitalize(about?.first_name?.[0]) : ''
                }${
                  about?.last_name ? Capitalize(about?.last_name?.[0]) : ''
                }`.trim()}
                radius={50}
                size={40}
                fontSize={10}
