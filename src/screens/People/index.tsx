...
export default function People({navigation}: RootScreenProps) {
  ...
  const {data: active_BD, isLoading: loadingActiveBD} = useFetchBirthdays(
    selected === 'Celebrations' ? 'active' : '',
  ) as useFetchBirthdaysProps;

  const {data: upcoming_BD, isLoading: loadingUpcomingBD} = useFetchBirthdays(
  ...
  const mapStateToData = () => {
    ...
    if (
      (selected === 'Celebrations' &&
        active_BD?.results &&
        Array.isArray(active_BD?.results)) ||
        ...
        active_BD?.results && Array.isArray(active_BD.results)
          ? active_BD.results.map((item) => ({
              ...
              date: item.birth_date
                ? moment(item.birth_date).format('MMM DD')
                : '',
            }))
          : [];

      let upcoming_birthdays: CelebrationItem[] =
        upcoming_BD?.results && Array.isArray(upcoming_BD.results)
      ...
      let upcoming_ann: CelebrationItem[] =
        upcomingANN?.results && Array.isArray(upcomingANN?.results)
          ? upcomingANN?.results.map((item) => ({
              title:
                item && item.first_name
                  ? `${Capitalize(item.first_name)}’s ${
                      item && item.num_years_spent ? item.num_years_spent : 0
                    } year anniversary`
                  : '',
              ...
              date: item.hire_date
                ? moment(item.hire_date).format('MMM DD')
                : '',
            }))
          : [];

      let active_ann: CelebrationItem[] =
        activeANN?.results && Array.isArray(activeANN.results)
      ...
      let celeb = [
        active_birthdays &&
        Array.isArray(active_birthdays) &&
        active_birthdays.length > 0
          ? {
              key: '1',
        ...
        active_ann && Array.isArray(active_ann) && active_ann.length > 0
          ...
          : null,
        upcoming_birthdays &&
        Array.isArray(upcoming_birthdays) &&
    ...
    if (
      ...
      let data: WhosOutListItem[] =
        outData?.results && Array.isArray(outData?.results)
          ...
          : [];
      let whos_out_data: WhosOutSection[] =
  ...
  const RenderEmployeeComponent = ({item}: {item: useFetchEmployeesData}) => {
    ...
    return <PersonCard item={item} onPressHandler={onPressHandler} />;
  };
  ...
  const RenderListViewComponent = ({item}: {item: useFetchEmployeesData}) => {
    ...
    return <PersonListComp item={item} onPressHandler={onPressHandler} />;
  };

  const KeyExtractor = (item: useFetchEmployeesData, index: number) =>
    `${index}${item}`.toString();
  ...
  const CelebrationKeyExtractor = (item: CelebrationItem, index: number) =>
    `${index}${item}`.toString();
  const onEndReached = () => {
    if (
          ...
          (fetchingTeams ||
            ...
            <React.Fragment>
              {selected === 'Celebrations' && celebrations ? (
                <SectionList
                  sections={
                    celebrations && Array.isArray(celebrations)
                      ? celebrations
                      : []
                  }
              ...
              {(selected === 'All' || selected === 'My Team') &&
              personsList &&
              Array.isArray(personsList) &&
