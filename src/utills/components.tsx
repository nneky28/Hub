...
export const ListComponent = ({index, item, onPress}: ListComponentProps) => {
  ...
  return (
    <TouchableWrapper
      ...
      key={index}
      style={styles.list_component}>
      <P color={AppColors.black1}>{name ? Capitalize(name) : ''}</P>
    </TouchableWrapper>
  );
};

export const ItemListModal = ({
  setOpen,
  loading,
  data,
  open,
                  ...
                  <CustomInput
                    ...
                    placeholder={
                      type === 'Contact'
                        ? 'First Name'
                        : type
                        ? Capitalize(type.toString().replace('_', ' '))
                        : ''
                    }
...
export const AppButton = (props: AppButtonProp) => (
  <TouchableOpacity onPress={props.onPress}>
    <Container
      ...
      {props.loading ? (
        <ActivityIndicator size={height(3)} color={AppColors.white} />
      ) : (
...
export const ImgPlaceholder = React.memo(
  (props: ImgPlaceholderProps) => (
    <Rounded
      size={props.size || 10}
      backgroundColor={
        props.backgroundColor || ColorList[Math.floor(Math.random() * 4)]
      }
      radius={props.radius || 10}>
      <H1 fontSize={props.fontSize} bold={props.bold} color={props.color}>
        {props.text}
      </H1>
    </Rounded>
  ),
...
export const DatePickerModal = (props: DatePickerModalProps) => {
  const [selected, setSelected] = React.useState<string | Date>(
      ...
      <Container
        ...
        <Container width={90} alignSelf="center">
          ...
          <Container
            ...
            <TouchableWrapper onPress={() => props.setShow(false)} isText>
...
const onNavigationStateChange = async ({
  ...
  try {
    let split =
      param.url && typeof param.url == 'string'
        ? param.url.split('&business_id')
        : [];
    ...
    let about_me = (await APIFunction.about_me(
      payload?.business_id,
    )) as useFetchAboutMeData;
...
export const OnboardModal = (props: OnboardModalProps) => {
  ...
  return (
    <Modal visible={props.visible}>
      <Container flex={1}>
        <Container marginTop={5} marginLeft={2} width={width(3)}>
          <TouchWrap
            onPress={() => {
        ...
        <WebView
          ...
          onNavigationStateChange={(param) =>
            onNavigationStateChange({param, dispatch, auth})
          }
...
export const CustomWebView = (props: CustomWebViewProps) => (
  <Modal visible={props.show}>
    <Container flex={1}>
      <Container marginTop={5} marginLeft={2} width={20}>
        <TouchWrap onPress={() => props.setShow()}>
...
export const CustomFallBackScreen = (props: CustomFallBackScreenProps) => {
  const {mutateAsync: reportError} = useMutation(APIFunction.error_report);
    ...
    <Container
      ...
      <Container width={90}>
        <EmptyStateWrapper
          ...
          sub_text={
            'This issue has been reported and our developers are working to resolve it.Please press refresh to login again.'
          }
        />
        <Container marginTop={5}>
          <Button title={'Refresh Now'} onPress={logoutMethod} />
        </Container>
      </Container>
    </Container>
  );
...
export const EmptyStateWrapper = React.memo(
  (props: EmptyStateWrapperProps) => {
    return (
      <Container
        ...
        {props.header_1 ? (
          <H1 color={AppColors.black3} fontSize={5} textAlign="center">
            {props.header_1}
          </H1>
        ) : null}
        <SizedBox height={props?.spacing || 2} />
        {props.header_2 ? (
          <React.Fragment>
            <H1 color={AppColors.black3} textAlign="center" fontSize={5}>
              {props.header_2}
            </H1>
            <SizedBox height={props?.spacing || 2} />
          </React.Fragment>
  ...
  (prevProps, nextProps) => {
    return (
      prevProps.header_1 === nextProps.header_1 &&
      prevProps.header_2 === nextProps.header_2 &&
  ...
  return (
    <React.Fragment>
      <Container horizontalAlignment="center" marginTop={4}>
        {!props?.hasPIN && props?.action === 'create' ? (
          <React.Fragment>
            <H1 fontSize={6} color={AppColors.black1}>
              Create New Pin
            </H1>
            <P color={AppColors.black1} style={CommonStyles.marginTop_1}>
              Stay sharp & secure with your pin
            </P>
          </React.Fragment>
        ) : null}

        {!props?.hasPIN && props?.action === 'confirm' ? (
          <React.Fragment>
            <H1 fontSize={6} color={AppColors.black1}>
              Confirm New Pin
            </H1>
            {props?.error ? (
              <P color={AppColors.red} style={CommonStyles.marginTop_1}>
                {props?.error}
              </P>
            ) : null}
            <P color={AppColors.black1} style={CommonStyles.marginTop_1}>
              Enter your pin again for confirmation
            </P>
          </React.Fragment>
        ) : null}
        {props?.hasPIN ? (
          <React.Fragment>
            ...
            <H1
              ...
              {props?.auth?.user?.first_name
                ? Capitalize(props?.auth?.user?.first_name)
                : null}{' '}
              {props?.auth?.user?.last_name
                ? `${Capitalize(props?.auth?.user?.last_name?.[0])}.`
                : null}
            </H1>
            {props?.error ? (
              <P color={AppColors.red} style={CommonStyles.marginTop_1}>
